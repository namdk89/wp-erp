...
Vue.component( 'new-note', {
    ...
    template: '#erp-tlm-new-note-template',
...
Vue.component( 'log-activity', {
    ...
    template: '#erp-tlm-log-activity-template',
    ...
    compiled: function() {
        this.feedData.inviteCourse = this.feedData.invite_course ? this.feedData.invite_course : [] ;
    },
    ...
    events: {
        'bindEditFeedData': function (feed ) {
            ...
            if ( feed.log_type == 'meeting' && ! _.isEmpty( feed.extra.invited_user ) ) {
                ...
                this.feedData.inviteCourse = invitedUser;
...
Vue.component( 'tasks-note', {
    ...
    template: '#erp-tlm-tasks-note-template',
    ...
    events: {
        'bindEditFeedData': function (feed ) {
            ...
            this.feedData.tp            = wperp.timeFormat( feed.start_date );
            var invitedUser             = feed.extra.invited_user.map( function( elm ) { return elm.id } );
            this.feedData.inviteCourse = invitedUser ? invitedUser : [];
    ...
    computed: {
        ...
        validation: function() {
            return {
                ...
                log_date : !!this.feedData.dt,
                log_time : !!this.feedData.tp,
                inviteCourse : ( this.feedData.inviteCourse ) && this.feedData.inviteCourse.length > 0 ? true : false
            }
        },
    ...
    watch: {
        feedData: {
            ...
            immediate: true,
            handler: function () {
                // if ( this.feedData.inviteCourse == null ) {
                //     this.feedData.inviteCourse = [];
                // }
                this.notify();
            }
        }
...
Vue.component( 'email-note', {
    ...
    template: '#erp-tlm-email-note-template',
    ...
    methods: {
        ...
        insertSaveReplies: function() {
            ...
            var data = {
                action      : 'erp-tlm-load-save-replies-data',
                template_id : this.emailTemplates,
                course_id  : this.feedData.user_id,
                _wpnonce    : wpTLMvue.nonce
            };

            jQuery.post( wpTLMvue.ajaxurl, data, function( resp ) {
                if ( resp.success ) {
        ...
        fileUpload: function() {
            ...
            $( '.tlm-attachments' ).css( 'display', 'block' );
...
Vue.component( 'schedule-note', {
    props: ['feed'],
    template: '#erp-tlm-schedule-note-template',
...
var vm = new Vue({
    ...
    methods: {
        ...
        loadMoreContent: function( feeds ) {
            ...
            var data = {
                action : 'erp_tlm_get_customer_activity',
        ...
        addCustomerFeed: function( comp, feed_id ) {
            ...
            if ( feed_id ) {
                ...
                vm.progressStart('#erp-tlm-feed-nav-content');
            }

            self.feedData._wpnonce = wpTLMvue.nonce;

            if ( self.feedData.type == 'log_activity' ) {
                self.feedData.log_date = self.feedData.dt;
                self.feedData.log_time = self.feedData.tp;
                self.feedData.invite_course = self.feedData.inviteCourse;
            };
            ...
            jQuery.post( wpTLMvue.ajaxurl, self.feedData, function( resp ) {
                if ( ! resp.success ) {
                    alert( resp.data );
                    vm.progreassDone(true);
                    return false;
                ...
                if ( self.feedData.id ) {
                    ...
                    document.getElementById("erp-tlm-activity-feed-form").reset();
                    ...
                    if ( vm.feedData.type == 'schedule' ) {
                        jQuery('#erp-tlm-activity-invite-course').select2().select2( "val", "" );
                ...
                $( '.tlm-attachments' ).css( 'display', 'none' );
            } );
        },
        ...
        fetchFeeds: function( filter ) {
            ...
            if ( filter ) {
                vm.progressStart('#erp-tlm-activities-filter');
        ...
        setLocaliziString: function() {
            ...
            data = {
                action: 'erp_tlm_set_localize_string'
            };
