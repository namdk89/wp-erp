 ...
 * Settings class
 */
class TLM_Settings extends ERP_Settings_Page {
    ...
    public function get_section_fields( $section = '' ) {
        ...
        $fields['courses'][] = [
            'title' => __( 'Course Settings', 'erp' ),
            'type'  => 'title',
            'desc'  => __( 'Settings for TLM Course.', 'erp' ),
            'id'    => 'general_options'
        ];
        ...
        $fields['courses'][] = [
            ...
            'id'      => 'user_auto_import',
            'type'    => 'select',
            'desc'    => __( 'Allow to auto import new user as tlm course.', 'erp' ),
        ...
        $fields['courses'][] = [
            'title'   => __( 'Default Course Owner', 'erp' ),
            'id'      => 'course_owner',
            'type'    => 'select',
            'desc'    => __( 'Default course owner for course.', 'erp' ),
            'options' => $users,
        ];
        ...
        $fields['courses'][] = [
            ...
            'id'      => 'life_stage',
            'type'    => 'select',
            'desc'    => __( 'Default life stage for course.', 'erp' ),
            'options' => $life_stages,
        ];
        ...
        $fields['subscription'][] = [
            'title' => __( 'Course Group subscription settings', 'erp' ),
            'type'  => 'title',
            'id'    => 'general_options'
        ];
        ...
        $fields['subscription'][] = [
            ...
            'desc'    => sprintf( __( "Don't forget to include: <code>[activation_link]Confirm your subscription.[/activation_link]</code>. <br><br>Optional: <code>[course_groups_to_confirm]</code>.", 'erp' ) )
        ];
        ...
        $fields = apply_filters( 'erp_settings_tlm_section_fields', $fields, $section );

        $section = $section === false ? $fields['courses'] : $fields[$section];
    ...
    public function get_email_connect_fields(){
        ...
        $fields[] = [
            ...
            'type'  => 'title',
            'desc'  => __( 'Settings for TLM Course Emails Connectivity.', 'erp' ),
            'id'    => 'general_options'
        ];
    ...
    protected function get_imap_settings_fields() {
        if ( !extension_loaded( 'imap' ) || !function_exists( 'imap_open' ) ) {
            $fields[] = [
                ...
                'type'  => 'title',
                'desc'  => sprintf(
                    '%s' . __( 'Your server does not have PHP IMAP extension loaded. To enable this feature, please course your hosting provider and ask to enable PHP IMAP extension.', 'erp' ) . '%s',
    ...
    public function imap_status() {
        $options = get_option( 'erp_settings_erp-tlm_email_connect_imap', [] );
    ...
    public function render_email_providers(){
        ...
        $settings_url = admin_url( 'admin.php?page=erp-settings&tab=erp-tlm&section=email_connect&sub_section=');
    ...
    public function listing_save_templates() {
        $save_replies = erp_tlm_get_save_replies();
    ?>
    <style type="text/css">
        td.erp-tlm-templates-wrapper {
            padding: 0 15px 10px 0;;
        }
        table.erp-tlm-templates-table th {
            padding: 9px 7px!important;
            vertical-align: middle;
        }

        table.erp-tlm-templates-table td {
            padding: 7px;
            line-height: 2em;
            vertical-align: middle;
        }

        table.erp-tlm-templates-table th.erp-templates-settings-table-name,
        table.erp-tlm-templates-table td.erp-templates-settings-table-name {
            padding-left: 15px !important;
        }

        table.erp-tlm-templates-table td.erp-templates-settings-table-name a {
            font-weight: 700;
        }

        table.erp-tlm-templates-table td.erp-templates-settings-table-actions{
            text-align: center;
        }
        table.erp-tlm-templates-table td.erp-templates-settings-table-actions a{
            margin-right: 8px;
        }

        table.erp-tlm-templates-table tr:nth-child(odd) td {
            background: #f9f9f9;
        }

        #erp-tlm-add-save-replies {
            margin-right: 15px;
            margin-bottom: 10px;
        }
    </style>
    <a href="#" class="erp-tlm-add-save-replies button alignright" id="erp-tlm-add-save-replies" title="<?php esc_attr_e( 'Add new Template', 'erp' ); ?>"><?php esc_attr_e( 'Add Templates', 'erp' ); ?></a>
    <tr valign="top">
        <td class="erp-tlm-templates-wrapper" colspan="2">
            <table class="erp-tlm-templates-table widefat" cellspacing="0">
                <thead>
                    <tr>
                        <?php
                        ...
                        foreach ( $save_replies as $replies_key => $save_reply ) {
                            ...
                            foreach ( $columns as $key => $column ) {
                                switch ( $key ) {
                                    ...
                                    case 'actions' :
                                        echo '<td class="erp-templates-settings-table-' . esc_attr( $key ) . '">
                                            <a class="erp-tlm-save-replies-edit erp-tips" title="'. esc_html__( 'Edit', 'erp' ) .'" href="#" data-id="' . esc_attr( $save_reply->id ) . '"><i class="fa fa-pencil-square-o"></i></a>
                                            <a class="erp-tlm-delete-save-replies erp-tips" title="'. esc_html__( 'Delete', 'erp' ) .'" href="#" data-id="' . esc_attr( $save_reply->id ) . '"><i class="fa fa-trash-o"></i></a>
                                        </td>';
                                        break;
        ...
        switch ( $section ) {
            case 'gmail' :
                if ( wperp()->google_auth->is_active() ) {
                    $option = get_option( 'erp_settings_erp-tlm_email_connect_imap', [] );
                    $option['enable_imap'] = 'no';
                    update_option( 'erp_settings_erp-tlm_email_connect_imap', $option );
                }
                break;
    ...
    function save( $section = false ) {
        if ( isset( $_POST['_wpnonce'] ) && wp_verify_nonce( sanitize_key( wp_unslash( $_POST['_wpnonce'] ) ), 'erp-settings-nonce' ) ) {
            ...
            if ( $current_section ) {
                ...
                do_action('erp_settings_tlm_updated_sub_section', $current_section, $update_options );
            } else {
                parent::save();
            }
        }
    ...
    public function cron_schedule( $value ) {
        ...
        wp_clear_scheduled_hook( 'erp_tlm_inbound_email_scheduled_events' );
        wp_schedule_event( time(), $recurrence, 'erp_tlm_inbound_email_scheduled_events' );
    }
}
