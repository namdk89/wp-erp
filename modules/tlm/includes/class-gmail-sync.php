        ...
        foreach ( $emails as $email ) {
            ...
            if ( isset( $email['headers']['References'] ) && preg_match( '/<' . $email_regexp . '>/', $email['headers']['References'], $matches ) ) {
                ...
                switch ( $message_id_parts[3] ) {
                    case 'r1':
                        $customer_feed_data = erp_tlm_save_email_activity( $email, $this->get_inbound_email() );
                        break;
                    case 'r2':
                        $customer_feed_data = erp_tlm_save_course_owner_email_activity( $email, $this->get_inbound_email() );
                        break;
                }
