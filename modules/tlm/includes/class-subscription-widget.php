...
class Subscription_Widget extends \WP_Widget {
    ...
    public function form( $instance ) {
        ...
        $erp_course_groups = erp_tlm_get_course_groups( $args );
        ...
        $defaults = [
            ...
            'course_groups'        => [],
            'life_stage'            => erp_get_option( 'life_stage', 'erp_settings_erp-tlm_courses', 'subscriber' ),
            'show_name_fields'      => 'no',
        ...
        $selected_groups    = $instance['course_groups'];
        ...
        <label><?php esc_attr_e( 'Course Groups' ); ?>:</label>
        <ul style="margin-top: 5px;">
            <?php foreach ( $erp_course_groups as $group ): ?>
                <li>
                    <label>
                        <input type="checkbox" class="checkbox" name="<?php echo esc_attr( $this->get_field_name( 'course_groups[]' ) ); ?>" value="<?php echo esc_attr( $group->id ); ?>" <?php echo in_array( $group->id, $selected_groups ) ? 'checked' : ''; ?> >
        ...
        <p>
            <label>
                ...
                <select name="<?php echo esc_attr( $this->get_field_name( 'life_stage' ) ); ?>" class="widefat">
                    <?php echo wp_kses( erp_tlm_get_life_stages_dropdown( [], $life_stage ), array(
                        'option' => array(
                            'value' => array(),
                            'selected' => array()
                        ),
    ...
    public function update( $new_instance, $old_instance ) {
        ...
        $instance['course_groups']   = ! empty( $new_instance['course_groups'] ) ? $new_instance['course_groups'] : [];
    ...
    public function widget( $args, $instance ) {
        ...
        $attrs = [
            'group'  => ! empty( $instance['course_groups'] ) ? $instance['course_groups'] : [],
