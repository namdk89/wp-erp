...
class Subscription {
    ...
    /**
     * TLM people_id for the subscriber
     *
     * @since 1.2.2
     *
     * @var integer
    ...
    public function register_widget() {
        register_widget( '\WeDevs\ERP\TLM\Subscription_Widget' );
    }
    ...
    public function wp_enqueue_scripts() {
        wp_enqueue_style( 'erp-subscription-form', WPERP_TLM_ASSETS . '/css/erp-subscription-form.css', [], WPERP_VERSION );
        ...
        wp_enqueue_script( 'erp-subscription-form', WPERP_TLM_ASSETS . '/js/erp-subscription-form.js', [ 'jquery' ], WPERP_VERSION, true );
    ...
    public function subscription_form( $args ) {
        ...
        foreach ( $groups as $group_id ) {
            ...
            $group = Models\CourseGroup::find( $group_id );
        ...
        $subscription_form_tmplt = apply_filters( 'erp_subscription_form_template', WPERP_TLM_VIEWS . '/subscription-form.php', $args );
    ...
    public function save_form_data() {
        ...
        if ( empty( $form_data['course']['email'] ) || ! is_email( $form_data['course']['email'] ) ) {
    ...
    /**
     * Create course group subscriber
     *
    ...
    public function create_subsciber( $args ) {
        $default_owner      = erp_tlm_get_default_course_owner();
        $default_life_stage = erp_get_option( 'life_stage', 'erp_settings_erp-tlm_courses', 'subscriber' );

        if ( ! empty( $args['life_stage'] ) ) {
            $registered_life_stages = erp_tlm_get_life_stages_dropdown_raw();
        ...
        if ( ! empty( $args['course']['full_name'] ) ) {
            $name_arr = explode( ' ', $args['course']['full_name'] );
        ...
        } else if ( ! empty( $args['course']['first_name'] ) ){
            $course['first_name'] = $args['course']['first_name'];
            $course['last_name']  = isset( $args['course']['last_name'] ) ? $args['course']['last_name'] : '';
        }

        $course = apply_filters( 'erp_subscription_form_save_form_data_args', $course, $args );

        // check if any people exists with this email
        $existing_course = erp_get_people_by( 'email', $course['email'] );

        // for existing course override the $_POST values with existing values
        if ( ! empty( $existing_course ) )  {
            foreach ( $course as $prop => $value ) {
                if ( ! empty( $existing_course->$prop ) ) {
                    $course[ $prop ] = $existing_course->$prop;
                }
            }
        ...
        $course_id = erp_insert_people( $course );

        if ( is_wp_error( $course_id ) ) {
            return new \WP_Error( 'error-insert-people', __( 'Unable to save data, please try again', 'erp' ) );
        }

        $course = new \WeDevs\ERP\TLM\Course( absint( $course_id ), 'course' );

        // insert metadata for new course
        if ( empty( $existing_course ) ) {
            $course->update_meta( 'source', 'optin_form' );
            $course->update_meta( 'course_owner', $default_owner );
        }

        $is_double_optin_enabled = erp_get_option( 'is_enabled', 'erp_settings_erp-tlm_subscription', 'yes' );
        $is_double_optin_enabled = filter_var( $is_double_optin_enabled, FILTER_VALIDATE_BOOLEAN );

        // subscribe to course group
        $subscribed_groups = [];
        foreach ( $args['groups'] as $group_id ) {
            $course_group = Models\CourseGroup::find( $group_id );

            if ( empty( $course_group ) ) {
                continue;
            }

            $existing_subscriber = Models\CourseSubscriber::where( [
                'user_id'  => $course_id,
                'group_id' => $group_id
            ] )->first();

            $hash = sha1( microtime() . 'erp-subscription-form' . $group_id . $course_id );
            ...
            if ( empty( $existing_subscriber ) ) {
                ...
                $subscribed_groups[] = erp_tlm_create_new_course_subscriber( $subs_args );
        ...
        if ( $is_double_optin_enabled && ! empty( $subscribed_groups ) ) {
            $this->send_mail( $course, $subscribed_groups, $args );
        }

        // when course is existing and already subscribed to every groups given in settings
        if ( $existing_course && empty( $subscribed_groups ) ) {
            return 'already-subscribed';
        }

        do_action( 'erp_subscription_form_save_form_data', $course, $subscribed_groups, $args );
     ...
     *
     * @param object $course           \WeDevs\ERP\TLM\Course object
     * @param array  $subscribed_groups Array of CourseSubscriber models
     * @param array  $form_data         Submitted form data
     *
     * @return void
     */
    private function send_mail( $course, $subscribed_groups, $args ) {
        ...
        $content_default = sprintf(
            __( "Hello!\n\nThanks so much for signing up for our newsletter.\nWe need you to activate your subscription to the list(s): [course_groups_to_confirm] by clicking the link below: \n\n[activation_link]Click here to confirm your subscription.[/activation_link]\n\nThank you,\n\n%s", 'erp' ),
            get_bloginfo( 'name' )
        );

        $to      = $course->data->email;
        $subject = erp_get_option( 'email_subject', 'erp_settings_erp-tlm_subscription', $subject_default );
        $content = erp_get_option( 'email_content', 'erp_settings_erp-tlm_subscription', $content_default );

        if ( preg_match( '/\[course_groups_to_confirm\]/', $content ) ) {
            ...
            $content = preg_replace( '/\[course_groups_to_confirm\]/', $group_names, $content );
        }
     ...
     *
     * @param array $groups Array of CourseSubscriber models
     *
     * @return string
     */
    public function get_confirmation_page_url( $groups ) {
        $page_id = erp_get_option( 'page_id', 'erp_settings_erp-tlm_subscription', 0 );
    ...
    public function subscription_page_frontend( $query ) {
        if ( $query->is_main_query() && ! empty( $_GET['erp-subscription-action'] ) ) {
            ...
            $this->sub_page_id = absint( erp_get_option( 'page_id', 'erp_settings_erp-tlm_subscription', 0 ) );
            ...
            if ( ! empty( $this->sub_page_id ) && absint( $page->ID ) === $this->sub_page_id ) {
                ...
                if ( ! empty( $_GET['subscription-id'] ) ) {
                    ...
                    $this->subscribed_groups = Models\CourseSubscriber::whereIn( 'hash', $subscription_ids )->get();
                    ...
                    switch ( $_GET['erp-subscription-action'] ) {
                        case 'edit':
                            ...
                            wp_enqueue_script( 'erp-subscription-edit', WPERP_TLM_ASSETS . '/js/erp-subscription-edit.js', [ 'jquery' ], WPERP_VERSION, true );
    ...
    private function confirm_subscription() {
        foreach ( $this->subscribed_groups as $group ) {
            ...
            do_action( 'erp_tlm_edit_course_subscriber', $group );
        }

        $people_id = $this->subscribed_groups->first()->user_id;

        $course = new Course( $people_id );
        if ( ! $course->hash ) {
     ...
     * @return void
     */
    private function unsubscribe_course() {
        if ( ! empty( $_GET['g'] ) ) {
            $group_ids = explode( ':', sanitize_text_field( wp_unslash( $_GET['g'] ) ) );

            $groups = Models\CourseGroup::whereIn( 'id', $group_ids )->get();

            if ( $groups->count() ) {
                $groups->each( function ( $group ) {
                    if ( empty( $group->private ) ) {
                        $subscriber = $group->course_subscriber()->where( 'user_id', $this->people_id )->first();
    ...
    public function subscription_page_title( $title, $id ) {
        ...
        switch ( $this->page_action ) {
            case 'confirm':
                $title = erp_get_option( 'confirm_page_title', 'erp_settings_erp-tlm_subscription', __( 'You are now subscribed!', 'erp' ) );
                break;

            case 'edit':
                $title = erp_get_option( 'edit_sub_page_title', 'erp_settings_erp-tlm_subscription', __( 'Edit Your Subscription', 'erp' ) );
                break;

            case 'unsubscribe':
            default:
                $title = erp_get_option( 'unsubs_page_title', 'erp_settings_erp-tlm_subscription', __( 'You are now unsubscribed', 'erp' ) );
                break;
        }
    ...
    public function subscription_page_content( $content ) {
        ...
        switch ( $this->page_action ) {
            case 'confirm':
                $content = erp_get_option( 'confirm_page_content', 'erp_settings_erp-tlm_subscription', __( "We've added you to our email list. You'll hear from us shortly.", 'erp' ) );
                break;

            case 'edit':
                $page_content   = erp_get_option( 'edit_sub_page_content', 'erp_settings_erp-tlm_subscription', __( 'Update your preferences', 'erp' ) );
                $template       = apply_filters( 'erp_subscription_edit_page_template', WPERP_TLM_VIEWS . '/edit-subscription-form.php' );
                $class_names    = apply_filters( 'erp_subscription_edit_page_template_class_names', ['erp-subscription-edit'] );
                $course_lists  = $this->get_lists_subscriber_belongs_to( $this->people_id );
                $hash           = $this->hash;
            ...
            case 'unsubscribe':
            default:
                $content = erp_get_option( 'unsubs_page_content', 'erp_settings_erp-tlm_subscription', __( 'You are successfully unsubscribed from list(s):', 'erp' ) );
    ...
    public function get_lists_subscriber_belongs_to( $people_id, $with_private = false ) {
        $lists = [
            'course_group' => Models\CourseGroup::getGroupSubscriber( $people_id, $with_private )->get()
        ];
    ...
    public function save_edit_form_data() {
        ...
        $course_lists = $this->get_lists_subscriber_belongs_to( $this->people_id, true );

        $course_groups = $course_lists['course_group'];
        $subscribed_groups = [];

        $course_groups->each( function ( $course_group ) use ( $form_data, &$subscribed_groups ) {
            if ( $course_group->private || ! empty( $form_data['course_group'][ $course_group->id ] ) ) {
                $subscribed_groups[] = $course_group->id;
            }
        } );

        erp_tlm_edit_course_subscriber( $subscribed_groups, $this->people_id );

        do_action( 'erp_subscription_edit', $form_data, $course_lists, $this );
