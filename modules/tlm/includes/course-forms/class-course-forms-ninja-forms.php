...
class Ninja_Forms {
    ...
    public function __construct() {
        ...
        $this->action( 'tlm_get_ninja_forms_forms', 'get_forms' );
                ...
                foreach ( $form_ids as $form_id ) {
                    ...
                    foreach ( $form->fields as $i => $field ) {
                        ...
                        if ( !empty( $saved_settings['ninja_forms'][ $form_id ]['map'][ $field['id'] ] ) ) {
                            $tlm_option = $saved_settings['ninja_forms'][ $form_id ]['map'][ $field['id'] ];
                        } else {
                            $tlm_option = '';
                        }

                        $forms[ $form_id ]['map'][ $field['id'] ] = !empty( $tlm_option ) ? $tlm_option : '';
                    }


                    if ( !empty( $saved_settings['ninja_forms'][ $form_id ]['course_group'] ) ) {
                        $forms[ $form_id ]['courseGroup'] = $saved_settings['ninja_forms'][ $form_id ]['course_group'];
                    }

                    if ( !empty( $saved_settings['ninja_forms'][ $form_id ]['course_owner'] ) ) {
                        $forms[ $form_id ]['courseOwner'] = $saved_settings['ninja_forms'][ $form_id ]['course_owner'];
                    }
                }
            }
            ...
            foreach ( $nf_forms as $i => $nform ) {
                ...
                foreach ( $fields as $i => $field ) {
                    ...
                    if ( !empty( $saved_settings['ninja_forms'][ $form_id ]['map'][ $field_id ] ) ) {
                        $tlm_option = $saved_settings['ninja_forms'][ $form_id ]['map'][ $field_id ];
                    } else {
                        $tlm_option = '';
                    }

                    $forms[ $form_id ]['map'][ $field_id ] = !empty( $tlm_option ) ? $tlm_option : '';
                }
