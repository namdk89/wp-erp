<?php
/**
 * TLM related necessary helper function
 */
 ...
 */
function erp_tlm_get_avatar( $id, $email = '', $user_id = 0, $size = 32 ) {
 ...
 * @return string  image with HTML tag
 */
function erp_tlm_get_avatar_url( $id, $email = '', $user_id = 0, $size = 32 ) {
 ...
 * @return object
 */
function erp_tlm_get_employees( $args = [] ) {
    global $wpdb;
    ...
    $cache_key = 'erp-tlm-get-employees-' . md5( serialize( $args ) );
 ...
 * @return html
 */
function erp_tlm_get_tlm_user_html_dropdown( $selected = '' ) {
    $tlm_users = erp_tlm_get_tlm_user();
    $dropdown  = '';

    if ( $tlm_users ) {
        foreach ( $tlm_users as $key => $user ) {
...
/**
 * Get course details url according to course type
 *
 * @since 1.0
 *
 * @param integer $id
 ...
 * @return string url
 */
function erp_tlm_get_details_url( $id, $type ) {

    if ( $id ) {

        if ( in_array( 'course', $type ) ) {
            return add_query_arg( ['page' => 'erp-tlm', 'section' => 'courses', 'action' => 'view' , 'id' => $id ], admin_url('admin.php') );
        }

        if ( in_array( 'company', $type ) ) {
            return add_query_arg( ['page' => 'erp-tlm', 'section' => 'companies', 'action' => 'view' , 'id' => $id ], admin_url('admin.php') );
        }
    }
 ...
 * @return array
 */
function erp_tlm_get_life_stages_dropdown_raw( $label = [], $counts = [] ) {
    ...
    $life_stages = [
        'initial'    => _n( 'Initital', 'Initital', $counts['initial'], 'erp' ),
        'learning'    => _n( 'Learning', 'Learning', $counts['learning'], 'erp' ),
        'pending'    => _n( 'Pending', 'Pending', $counts['pending'], 'erp' ),
        'dropped'    => _n( 'Dropped', 'Dropped', $counts['dropped'], 'erp' ),
        'completed'    => _n( 'Completed', 'Completed', $counts['completed'], 'erp' )
    ];

    $life_stages = apply_filters( 'erp_tlm_life_stages', $life_stages );
 ...
 * @return array
 */
function erp_tlm_get_customer_type( $label = [] ) {
    ...
    return apply_filters( 'erp_tlm_customer_type', $type );
}
 ...
 * @return html
 */
function erp_tlm_get_life_stages_dropdown( $label = [], $selected = '' ) {

    $life_stages = erp_tlm_get_life_stages_dropdown_raw( $label );
    $dropdown    = '';
...
/**
 * Get course dropdown list as array
 *
 * @since 1.0
 *
 * @param  array $label
 *
 * @return array | list of all course
 */
function erp_tlm_get_course_dropdown( $label = [] ) {
    $courses = erp_get_peoples( [ 'number' => '-1', 'type' => [ 'customer' ] ] );
    $list     = [];

    foreach ( $courses as $key => $course ) {
        $course_obj              = new \WeDevs\ERP\TLM\Course( intval( $course->id ) );
        $list[ $course_obj->id ] = $course_obj->get_full_name() . '( ' . $course_obj->get_email() . ' ) ';
    }
...
/**
 * Get course dropdown list options
 *
 * @since 1.0
 *
 * @return array | list of all course
 */
function erp_tlm_get_course_options() {
    $purchased = erp_acct_get_all_invoices(['status' => '4']);
    $received = erp_acct_get_sales_transactions(['status' => '4']);

    $dropdown  = sprintf( '<option value="">%s</option>', __( '--Select--', 'erp' ) );
    $courses = erp_get_peoples( [ 'number' => '-1', 'type' => [ 'customer' ] ] );

    foreach ($courses as $key => $course) {
        $course_obj = new \WeDevs\ERP\TLM\Course( intval( $course->id ) );
        $value = $course_obj->get_full_name() . '( ' . $course_obj->get_email() . ' ) ';
...
/**
 * Get company dropdown list as array
 *
 * @since 1.0
 *
 * @param  array $label
 *
 * @return array | list of all company with copmany
 */
function erp_tlm_get_company_dropdown( $label = [] ) {
    $companies = erp_get_peoples( [ 'number' => '-1', 'type' => [ 'company' ] ] );
    $list     = [];

    foreach ( $companies as $key => $company ) {
        $company_obj              = new \WeDevs\ERP\TLM\Course( intval( $company->id ) );
        $list[ $company_obj->id ] = $company_obj->get_full_name() . '( ' . $company_obj->get_email() . ' ) ';
    }
 ...
 * @return array
 */
function erp_tlm_customer_get_status_count( $type = null ) {
    global $wpdb;

    $cache_key = 'erp-tlm-customer-status-counts-' . $type;
    $results   = wp_cache_get( $cache_key, 'erp' );
    ...
    if ( false == $results ) {
        ...
        if ( current_user_can( 'erp_tlm_agent' ) ) {
            ...
            $sql .= " AND {$people_tbl}.course_owner = {$current_user_id}";
        }
    ...
    $statuses = erp_tlm_get_life_stages_dropdown_raw( [ 'all' => __( 'All', 'erp' ) ], $status_counts );
    ...
    $counts['trash'] = [
        'count' => erp_tlm_count_trashed_customers( $type ),
        'label' => __( 'Trash', 'erp' )
    ];
 ...
 */
function erp_tlm_count_trashed_customers( $type = null ) {
 ...
 * @return
 */
function erp_tlm_customer_add_company( $customer_id, $company_id ) {

    global $wpdb;

    $wpdb->insert( $wpdb->prefix . 'erp_tlm_customer_companies', array(
 ...
 * @return array
 */
function erp_tlm_customer_get_company( $postdata ) {
    global $wpdb;
    $results = [];

    if ( isset( $postdata['id'] ) && empty( $postdata['id'] ) ) {
        return new WP_Error( 'no-ids', __( 'No course found', 'erp' ) );
    }

    $sql = "SELECT com.* FROM " . $wpdb->prefix . "erp_tlm_customer_companies AS com
    ...
    if ( $data ) {
        foreach ( $data as $key => $value ) {
            $company                                       = new \WeDevs\ERP\TLM\Course( intval( $value['company_id'] ) );
            $results[ $key ]                               = $value;
            $results[ $key ]['course_details']            = $company->to_array();
            $country                                       = $results[ $key ]['course_details']['country'];
            $results[ $key ]['course_details']['country'] = erp_get_country_name( $country );
            $results[ $key ]['course_details']['state']   = erp_get_state_name( $country, $results[ $key ]['course_details']['state'] );
        }
    }
 ...
 * @return array
 */
function erp_tlm_company_get_customers( $postdata ) {
    global $wpdb;
    $results = [];
    ...
    if ( $data ) {
        foreach ( $data as $key => $value ) {
            $customer                                      = new \WeDevs\ERP\TLM\Course( intval( $value['customer_id'] ) );
            $results[ $key ]                               = $value;
            $results[ $key ]['course_details']            = $customer->to_array();
            $country                                       = $results[ $key ]['course_details']['country'];
            $results[ $key ]['course_details']['country'] = erp_get_country_name( $country );
            $results[ $key ]['course_details']['state']   = erp_get_state_name( $country, $results[ $key ]['course_details']['state'] );
        }
 ...
 */
function erp_tlm_get_customer_details_url( $id ) {
    return add_query_arg( ['page' => 'erp-tlm', 'section' => 'courses', 'action' => 'view' , 'id' => $id ], admin_url('admin.php') );
}
 ...
 * @return void
 */
function erp_tlm_customer_update_company( $row_id, $company_id ) {
    global $wpdb;
    $wpdb->update( $wpdb->prefix . "erp_tlm_customer_companies", [ 'company_id' => $company_id ], [ 'id' => $row_id ] );
}
 ...
 * @return mixed
 */
function erp_tlm_customer_remove_company( $id ) {
    global $wpdb;
    $wpdb->delete( $wpdb->prefix . 'erp_tlm_customer_companies', [ 'id' => $id ] );
}
 ...
 * @return array
 */
function erp_tlm_get_social_field() {
    ...
    return apply_filters( 'erp_tlm_social_field', $social_field );
}
 ...
 * @return array
 */
function erp_tlm_get_customer_feeds_nav() {
    return apply_filters( 'erp_tlm_customer_feeds_nav', [
 ...
 * @return null|array
 */
function erp_tlm_check_customer_exist_company( $customer_id, $company_id ) {
    global $wpdb;

    $sql = "SELECT `id` FROM {$wpdb->prefix}erp_tlm_customer_companies WHERE `customer_id` = '$customer_id' AND `company_id` = '$company_id'";
 ...
 * @return array
 */
function erp_tlm_customer_prepare_schedule_postdata( $postdata ) {
    if ( empty( $postdata ) ) {
        return;
    }
 ...
 * @return string
 */
function erp_tlm_format_activity_feed_message( $message, $activity ) {
    return apply_filters( 'erp_tlm_format_activity_feed_message', stripslashes( $message ), $activity );
}
 ...
 * @return array
 */
function erp_tlm_get_feed_activity( $postdata ) {
    global $wpdb;
    $feeds = [];
    $db    = new \WeDevs\ORM\Eloquent\Database();

    $results = \WeDevs\ERP\TLM\Models\Activity::select( [
        '*',
    ...
    if ( current_user_can( 'erp_tlm_agent' ) ) {
        $course_owner = get_current_user_id();
        $people_ids = array_keys( $wpdb->get_results( "SELECT id FROM {$wpdb->prefix}erp_peoples WHERE course_owner = {$course_owner}", OBJECT_K ) );
    ...
    foreach ( $results as $key => $value ) {
        ...
        if ( isset( $value['extra']['invite_course'] ) && ! empty( $postdata['assigned_to'] ) ) {
            if ( ! in_array( $postdata['assigned_to'], $value['extra']['invite_course'] ) ) {
                continue;
            }
        }

        if ( isset( $value['extra']['invite_course'] ) && count( $value['extra']['invite_course'] ) > 0 ) {
            foreach ( $value['extra']['invite_course'] as $user_id ) {
        ...
        if ( $value['course']['user_id'] ) {
            $value['course']['first_name'] = get_user_meta( $value['course']['user_id'], 'first_name', true );
            $value['course']['last_name']  = get_user_meta( $value['course']['user_id'], 'last_name', true );
        }

        if ( ! empty( $value['course']['types'] ) ) {
            $value['course']['types'] = wp_list_pluck( $value['course']['types'], 'name' );
        } else {
            $value['course']['types'] = [];
        }

        if ( isset( $value['extra']['attachments'] ) ) {
            $value['extra']['attachments'] = erp_tlm_process_attachment_data( $value['extra']['attachments'] );
        }

        unset( $value['extra']['invite_course'] );
        $value['message']               = erp_tlm_format_activity_feed_message( $value['message'], $value );
 ...
 * @return array
 */
function erp_tlm_save_customer_feed_data( $data ) {

    if ( isset( $data['id'] ) && ! empty( $data['id'] ) ) {
        $saved_activity    = WeDevs\ERP\TLM\Models\Activity::find( $data['id'] )->update( $data );
        $saved_activity_id = $data['id'];
    } else {
        $saved_activity    = WeDevs\ERP\TLM\Models\Activity::create( $data );
        $saved_activity_id = $saved_activity->id;
    }

    $activity = WeDevs\ERP\TLM\Models\Activity::
    with( [
        'course'    => function ( $query ) {
            $query->with( 'types' );
        },
    ...
    if ( isset( $activity['extra']['invite_course'] ) && count( $activity['extra']['invite_course'] ) > 0 ) {
        foreach ( $activity['extra']['invite_course'] as $user_id ) {
    ...
    if ( $activity['course']['user_id'] ) {
        $activity['course']['first_name'] = get_user_meta( $activity['course']['user_id'], 'first_name', true );
        $activity['course']['last_name']  = get_user_meta( $activity['course']['user_id'], 'last_name', true );
    }

    unset( $activity['extra']['invite_course'] );

    $activity['course']['types']      = wp_list_pluck( $activity['course']['types'], 'name' );
    $activity['message']               = erp_tlm_format_activity_feed_message( $activity['message'], $activity );
    ...
    if ( isset( $activity['extra']['attachments'] ) ) {
        $activity['extra']['attachments'] = erp_tlm_process_attachment_data( $activity['extra']['attachments'] );
    }
 ...
 * @return collection
 */
function erp_tlm_customer_get_single_activity_feed( $feed_id ) {
    ...
    $results = [];
    $data    = WeDevs\ERP\TLM\Models\Activity::with( [
        'course'    => function ( $query ) {
            $query->with( 'types' );
        },
        'created_by' => function ( $query1 ) {
    ...
    if ( isset( $data['extra']['invite_course'] ) && count( $data['extra']['invite_course'] ) > 0 ) {
        foreach ( $data['extra']['invite_course'] as $user_id ) {
    ...
    if ( $data['course']['user_id'] ) {
        $data['course']['first_name'] = get_user_meta( $data['course']['user_id'], 'first_name', true );
        $data['course']['last_name']  = get_user_meta( $data['course']['user_id'], 'last_name', true );
    }

    $data['course']['types'] = wp_list_pluck( $data['course']['types'], 'name' );
    $data['message']          = stripslashes( $data['message'] );

    if ( isset( $data['extra']['attachments'] ) ) {
        $data['extra']['attachments'] = erp_tlm_process_attachment_data( $data['extra']['attachments'] );
    }
 ...
 * @return mixed
 */
function erp_tlm_process_attachment_data( $attachments ) {
 ...
 * @return collection
 */
function erp_tlm_customer_delete_activity_feed( $feed_id ) {
    $activity = WeDevs\ERP\TLM\Models\Activity::find( $feed_id );

    if ( $activity->type == 'tasks' ) {
        WeDevs\ERP\TLM\Models\ActivityUser::where( 'activity_id', $activity->id )->delete();
    }
 ...
 * @return void
 */
function erp_tlm_customer_schedule_notification() {
    $schedules = \WeDevs\ERP\TLM\Models\Activity::schedules()->get()->toArray();
        ...
        if ( isset ( $extra['allow_notification'] ) && $extra['allow_notification'] == 'true' ) {
            if ( ( current_time( 'mysql' ) >= $extra['notification_datetime'] ) && ( $activity['start_date'] >= current_time( 'mysql' ) ) ) {
                if ( ! $activity['sent_notification'] ) {
                    erp_tlm_send_schedule_notification( $activity, $extra );
                }
            }
        }
    }
 ...
 * @return void
 */
function erp_tlm_send_schedule_notification( $activity, $extra = false ) {
    if ( ! $extra ) {
        return;
    }
    ...
    switch ( $extra['notification_via'] ) {
        case 'email':
            ...
            foreach ( $extra['invite_course'] as $course ) {
                $users[] = get_the_author_meta( 'user_email', $course );
            }
            ...
            erp_tlm_update_schedule_notification_flag( $activity['id'], true );
            break;

        default:
            do_action( 'erp_tlm_send_schedule_notification', $activity, $extra );
            break;
    }
}
 ...
 * @return void
 */
function erp_tlm_update_schedule_notification_flag( $activity_id, $flag ) {
    if ( ! $activity_id ) {
        return;
    }
    \WeDevs\ERP\TLM\Models\Activity::find( $activity_id )->update( [ 'sent_notification' => $flag ] );
}
 ...
 * @return void
 */
function erp_tlm_assign_task_to_users( $data, $save_data ) {
    if ( $save_data['id'] ) {
        \WeDevs\ERP\TLM\Models\ActivityUser::where( 'activity_id', $save_data['id'] )->delete();
    }
    ...
    if ( isset( $data['extra']['invited_user'] ) && count( $data['extra']['invited_user'] ) > 0 ) {
        foreach ( $data['extra']['invited_user'] as $key => $user ) {
            $res = \WeDevs\ERP\TLM\Models\ActivityUser::create( [
                'activity_id' => $data['id'],
                'user_id'     => $user['id']
            ] );
            ...
            do_action( 'erp_tlm_after_assign_task_to_user', $data, $save_data );
        }
 ...
 * @return array
 */
function erp_tlm_save_course_group( $data ) {
    if ( ! empty ( $data['id'] ) ) {
        $result = WeDevs\ERP\TLM\Models\CourseGroup::find( $data['id'] )->update( $data );
	    do_action( 'erp_tlm_update_course_group', $result );
    } else {
        $result = WeDevs\ERP\TLM\Models\CourseGroup::create( $data );
	    do_action( 'erp_tlm_create_course_group', $result );
    }
 ...
 * @return object
 */
function erp_tlm_get_course_groups( $args = [] ) {
    ...
    $args      = wp_parse_args( $args, $defaults );
    $cache_key = 'erp-tlm-course-group-' . md5( serialize( $args ) );
    $items     = wp_cache_get( $cache_key, 'erp' );

    if ( false === $items ) {
        // Check if args count true, then return total count customer according to above filter
        if ( $args['count'] ) {
            $result = WeDevs\ERP\TLM\Models\CourseGroup::count();
        ...
        $results       = [];
        $course_group = new WeDevs\ERP\TLM\Models\CourseGroup();

        $course_group = $course_group->with( 'course_subscriber' );

        // Check if want all data without any pagination
        if ( $args['number'] != '-1' && ! $args['count'] ) {
            $course_group = $course_group->skip( $args['offset'] )->take( $args['number'] );
        }

        // Check is the row want to search
        if ( isset( $args['s'] ) && ! empty( $args['s'] ) ) {
            $arg_s         = $args['s'];
            $course_group = $course_group->where( 'name', 'LIKE', "%$arg_s%" )
                                           ->orWhere( 'description', 'LIKE', "%$arg_s%" );
        }

        // Render all collection of data according to above filter (Main query)
        $results = $course_group->orderBy( $args['orderby'], $args['order'] )
                                 ->get()
                                 ->toArray();
        ...
        foreach ( $results as $key => $group ) {
            ...
            if ( ! current_user_can( 'erp_tlm_create_groups' ) ) {
                $course_subscriber = $group['course_subscriber'];
                $agent_subscriber   = [];
                foreach ( $course_subscriber as $cs ) {
                    $obj = erp_get_people( $cs['user_id'] );
                    if ( $obj->course_owner == get_current_user_id() ) {
                        $agent_subscriber[] = $cs;
                    }
                }
                $group['course_subscriber'] = $agent_subscriber;
            }

            $subscribers = array_filter( $group['course_subscriber'], function ( $subscriber ) {
                return 'subscribe' === $subscriber['status'];
            } );

            $unconfirmed = array_filter( $group['course_subscriber'], function ( $subscriber ) {
                return 'unconfirmed' === $subscriber['status'];
            } );

            $unsubscribers = array_filter( $group['course_subscriber'], function ( $subscriber ) {
                return $subscriber['unsubscribe_at'];
            } );

            unset( $group['course_subscriber'] );
...
/**
 * Get course group by its primary key[id]
 *
 * @since 1.0
 *
 * @param  integer $id
 ...
 * @return array
 */
function erp_tlm_get_course_group_by_id( $id ) {
    return WeDevs\ERP\TLM\Models\CourseGroup::find( $id )->toArray();
}
 ...
 * @throws \Exception
 */
function erp_tlm_course_group_delete( $id ) {
    if ( is_array( $id ) ) {
        WeDevs\ERP\TLM\Models\CourseGroup::destroy( $id );
    } else {
        WeDevs\ERP\TLM\Models\CourseGroup::find( $id )->delete();
    }

	do_action( 'erp_tlm_delete_course_group', $id );
}

/**
 * Get subscriber course
 *
 * @since 1.0
 *
 * @param  array $args
 ...
 * @return array|object
 */
function erp_tlm_get_subscriber_course( $args = [] ) {
    global $wpdb;
    ...
    $args      = wp_parse_args( $args, $defaults );
    $cache_key = 'erp-tlm-subscriber-course-' . md5( serialize( $args ) );
    $items     = wp_cache_get( $cache_key, 'erp' );

    if ( false === $items ) {
        $converted_data       = [];
        $course_subscribe_tb = $wpdb->prefix . 'erp_tlm_course_subscriber';
        $course_group_tb     = $wpdb->prefix . 'erp_tlm_course_group';
        $course_tags         = $wpdb->prefix . 'erp_tlm_course_tag';

        $course_subscribers = WeDevs\ERP\TLM\Models\CourseSubscriber::leftjoin( $course_group_tb, $course_group_tb . '.id', '=', $course_subscribe_tb . '.group_id' );

//        $course_subscribers = $course_subscribers::leftjoin('')
        if( ! current_user_can( 'erp_tlm_create_groups' ) ) {
            $erp_peoples         = $wpdb->prefix . 'erp_peoples';
            $course_subscribers = $course_subscribers->leftJoin( $erp_peoples, $erp_peoples . '.id', '=', $course_subscribe_tb . '.user_id' )->addSelect( $course_subscribe_tb . '.*', $course_group_tb . '.*', $erp_peoples . '.course_owner' )->where( $erp_peoples . '.course_owner', '=', get_current_user_id() );
        }

        // Check if want all data without any pagination
        if ( $args['number'] != '-1' && ! $args['count'] ) {
            $course_subscribers = $course_subscribers->skip( $args['offset'] )->take( $args['number'] );
        }

        if ( isset( $args['group_id'] ) && ! empty( $args['group_id'] ) ) {
            $course_subscribers = $course_subscribers->where( $course_group_tb . '.id', '=', $args['group_id'] );
        }

        // Check is the row want to search
        if ( isset( $args['s'] ) && ! empty( $args['s'] ) ) {
            $arg_s               = $args['s'];
            $course_subscribers = $course_subscribers->where( 'name', 'LIKE', "%$arg_s%" )
                                                       ->orWhere( 'description', 'LIKE', "%$arg_s%" );
        }

        // Render all collection of data according to above filter (Main query)
        $results = $course_subscribers
            ->get()
            ->groupBy( 'user_id' )
            ->toArray();
        ...
        /*if ( $args['count'] ) {
            if ( ! empty( $args['group_id'] ) ) {
                $items = WeDevs\ERP\TLM\Models\CourseSubscriber::leftjoin( $course_group_tb, $course_group_tb . '.id', '=', $course_subscribe_tb . '.group_id' )->where( $course_subscribe_tb . '.group_id', $args['group_id'] )->count();
            } else {
                $items = WeDevs\ERP\TLM\Models\CourseSubscriber::leftjoin( $course_group_tb, $course_group_tb . '.id', '=', $course_subscribe_tb . '.group_id' )->count();
            }
        }*/
 ...
 * @return array
 */
function erp_tlm_get_course_group_dropdown( $label = [] ) {
    $groups = erp_tlm_get_course_groups_list();
...
/**
 * Get already subscirbed course
 *
 * @since 1.0
 *
 * @return array
 */
function erp_tlm_get_assign_subscriber_course() {
    $data = \WeDevs\ERP\TLM\Models\CourseSubscriber::select( 'user_id' )->distinct()->get()->toArray();

    return wp_list_pluck( $data, 'user_id' );
}

/**
 * Create Course subscriber
 *
 * @since 1.0
 ...
 * @param  array $data
 *
 * @return return object CourseSubscriber model or WP_Error
 */
function erp_tlm_create_new_course_subscriber( $args = [] ) {
    $defaults = array(
    ...
    if ( empty( $args['user_id'] ) ) {
        return new WP_Error( 'user-id', __( 'No course founds', 'erp' ) );
    }

    $subscriber = \WeDevs\ERP\TLM\Models\CourseSubscriber::create( $args );

    $course = new \WeDevs\ERP\TLM\Course( $subscriber->user_id );
    $hash_id = sha1( microtime() . 'erp-subscription' . $args['group_id'] . $args['user_id'] );

    if ( ! $course->hash ) {
        $course->update_course_hash( $hash_id );
    }

    do_action( 'erp_tlm_create_course_subscriber', $subscriber, $hash_id );
 ...
 * @return array
 */
function erp_tlm_get_editable_assign_course( $user_id ) {
    $data = \WeDevs\ERP\TLM\Models\CourseSubscriber::where( 'user_id', $user_id )->distinct()->get()->toArray();
 ...
 * @return array
 */
function erp_tlm_get_user_assignable_groups( $user_id ) {
    if ( ! $user_id ) {
        return new WP_Error( 'no-user-id', __( 'No course found', 'erp' ) );
    }

    $data = \WeDevs\ERP\TLM\Models\CourseSubscriber::with( 'groups' )
                                                    ->where( [
                                                        'user_id' => $user_id,
                                                        'status'  => 'subscribe'
                                                    ] )
...
/**
 * Delete Course alreays subscribed
 *
 * @since 1.0
 *
 * @param  integer $id
 ...
 * @return boolean
 */
function erp_tlm_course_subscriber_delete( $id, $group_id ) {
    if ( empty( $id ) || empty( $group_id ) ) {
        return false;
    }

    do_action( 'erp_tlm_pre_unsubscribed_course', $id, $group_id );

    if ( is_array( $id ) ) {
        return \WeDevs\ERP\TLM\Models\CourseSubscriber::whereIn( 'user_id', $id )->where( 'group_id', $group_id )->delete();
    } else {
        return \WeDevs\ERP\TLM\Models\CourseSubscriber::where( 'user_id', $id )->where( 'group_id', $group_id )->delete();
    }
}

/**
 * Edit course subscriber
 *
 ...
 * @return void
 */
function erp_tlm_edit_course_subscriber( $groups, $user_id ) {
    $data = \WeDevs\ERP\TLM\Models\CourseSubscriber::where( 'user_id', $user_id )->distinct()->get()->toArray();
    ...
    if ( ! empty( $unsubscribe_group ) ) {
        foreach ( $unsubscribe_group as $unsubscribe_group_key => $unsubscribe_group_id ) {
            $updated = \WeDevs\ERP\TLM\Models\CourseSubscriber::where( 'user_id', $user_id )
            ...
            if ( $updated ) {
                $subscriber = \WeDevs\ERP\TLM\Models\CourseSubscriber::where( 'user_id', $user_id )
                                                                      ->where( 'group_id', $unsubscribe_group_id )
                                                                      ->where( 'status', 'subscribe' )
                                                                      ->first();

                do_action( 'erp_tlm_edit_course_subscriber', $subscriber );
            }
        }
    }
    ...
    if ( ! empty( $del_group ) ) {
        foreach ( $del_group as $del_group_key => $del_group_id ) {
            ...
            do_action( 'erp_tlm_delete_course_subscriber', $subscriber );
        }
    }
}

/**
 * Course Group subscription statuses
 *
 * @since 1.1.17
 *
 * @return array
 */
function erp_tlm_get_subscription_statuses() {
    return apply_filters( 'erp_tlm_get_subscription_statuses', [
 ...
 * @return array
 */
function erp_tlm_get_campaigns( $args = [] ) {
    ...
    $args      = wp_parse_args( $args, $defaults );
    $cache_key = 'erp-tlm-campaign-' . md5( serialize( $args ) );
    $items     = wp_cache_get( $cache_key, 'erp' );

    if ( false === $items ) {
        $campaigns = new \WeDevs\ERP\TLM\Models\Campaign();
        ...
        // Check if args count true, then return total count customer according to above filter
        if ( $args['count'] ) {
            $items = WeDevs\ERP\TLM\Models\Campaign::count();
        }
 ...
 * @return array
 */
function erp_tlm_get_serach_key( $type = '' ) {
    $fields = [
        ...
        'tags'        => [
            ...
            'options'   => erp_html_generate_dropdown(erp_tlm_get_course_tags())
        ],

        'course_group' => [
            'title'     => __( 'Course Group', 'erp' ),
            'type'      => 'dropdown',
            'text'      => '',
            'condition' => [
                ''   => __( 'in group', 'erp' ),
            ...
            'options'   => erp_html_generate_dropdown( wp_list_pluck( \WeDevs\ERP\TLM\Models\CourseGroup::select( 'id', 'name' )->get()->keyBy( 'id' )->toArray(), 'name' ) )
        ],
        ...
        'life_stage' => [
            ...
            'options'   => erp_tlm_get_life_stages_dropdown()
        ],

        'source' => [
            'title'     => __( 'Course Source', 'erp' ),
            'type'      => 'dropdown',
            'text'      => '',
            'condition' => [
                ''  => __( 'is', 'erp' ),
            ...
            'options'   => erp_tlm_course_source_dropdown()
        ],

        'course_age' => [
            'title'     => __( 'Course age', 'erp' ),
            'type'      => 'number_range',
            'text'      => '',
            'condition' => [
                ''   => __( 'exactly', 'erp' ),
    ...
    if ( 'course' == $type ) {
        $fields = erp_tlm_get_customer_serach_key() + $fields;
    }

    if ( 'company' == $type ) {
        $fields = erp_tlm_get_company_serach_key() + $fields;
    }

    return apply_filters( 'erp_tlm_global_serach_fields', $fields, $type );
}
 ...
 * @return array
 */
function erp_tlm_get_customer_serach_key() {
    return apply_filters( 'erp_tlm_customer_search_fields', [
        'first_name' => [
 ...
 * @return array
 */
function erp_tlm_get_company_serach_key() {
    return apply_filters( 'erp_tlm_company_search_fields', [
        'company' => [
 ...
 * @return array
 */
function erp_tlm_get_save_search_regx( $values ) {
    $result = [];
    ...
    return apply_filters( 'erp_tlm_get_save_search_regx', $result, $values );
}
 ...
 * @return bool
 */
function erp_tlm_check_segment_exists( $name ) {
    $exists = WeDevs\ERP\TLM\Models\SaveSearch::where( 'search_name', $name )->first();
 ...
 * @return array
 */
function erp_tlm_insert_save_search( $data ) {
    if ( $data['id'] ) {
        $updated_item = WeDevs\ERP\TLM\Models\SaveSearch::find( $data['id'] );
        $updated_item->update( $data );

        return $updated_item;
    } else {
        return WeDevs\ERP\TLM\Models\SaveSearch::create( $data );
    }
}
 ...
 * @return array
 */
function erp_tlm_get_save_search_item( $args = [] ) {
    ...
    if ( $args['id'] ) {
        return WeDevs\ERP\TLM\Models\SaveSearch::find( $args['id'] )->toArray();
    }

    $results     = [];
    $search_keys = WeDevs\ERP\TLM\Models\SaveSearch::where( 'user_id', '=', $args['user_id'] )
                                                    ->orWhere( 'global', '=', 1 );
 ...
 * @return boolean
 */
function erp_tlm_delete_save_search_item( $id ) {
    return WeDevs\ERP\TLM\Models\SaveSearch::find( $id )->delete();
}
 ...
 * @return string
 */
function erp_tlm_get_search_by_already_saved( $save_search_id ) {
    if ( ! $save_search_id ) {
        return '';
    }

    $data = WeDevs\ERP\TLM\Models\SaveSearch::find( $save_search_id );

    return $data->search_val;
}

/**
 * Advance filter for course and company
 *
 * @since 1.1.0
 *
 ...
 * @return array
 */
function erp_tlm_course_advance_filter( $custom_sql, $args ) {
    global $wpdb;
    ...
    $people_meta_fields = erp_tlm_get_course_meta_fields();
    ...
    $allowed    = erp_tlm_get_serach_key( $args['type'] );
    $query_data = [];
            ...
            if ( $or_query ) {
                ...
                foreach ( $or_query as $field => $value ) {
                    if ( in_array( $field, $pep_fileds ) ) {
                        if ( $value ) {
                            $val                   = erp_tlm_get_save_search_regx( $value );
                            $custom_sql['where'][] = "(";
                            $j                     = 0;
                        ...
                        foreach ( $value as $key => $search_value ) {
                            $search_condition_regx = erp_tlm_get_save_search_regx( $search_value );
                        ...
                        $custom_sql['where'][] = ( $i == count( $or_query ) - 1 ) ? ")" : " ) AND";
                    } else if ( $field == 'course_group' ) {
                        if ( ! $is_course_group_joined ) {
                            $custom_sql['join'][] = "LEFT JOIN {$wpdb->prefix}erp_tlm_course_subscriber as subscriber ON people.id = subscriber.user_id";

                            if ( ! $args['count'] ) {
                                $custom_sql['group_by'][] = 'people.id';
                            }

                            $is_course_group_joined = true;
                        }
                        ...
                        foreach ( $value as $j => $search ) {
                            $addOr                 = ( $j == count( $value ) - 1 ) ? '' : " OR ";
                            $search_condition_regx = erp_tlm_get_save_search_regx( $search );
                            $condition             = array_shift( $search_condition_regx );
                        ...
                        $custom_sql['where'][] = "AND term_taxonomy.taxonomy = 'erp_tlm_tag'";
                        $custom_sql['where'][] = ( $i == count( $or_query ) - 1 ) ? ")" : " ) AND";
                        ...
                        $custom_sql = apply_filters( 'erp_tlm_customer_segmentation_sql', $custom_sql, $field, $value, $or_query, $i, $table_alias );
                    }
 ...
 * @return array
 */
function erp_tlm_is_people_belongs_to_saved_search( $sql, $args ) {
 ...
 * @return array
 */
function erp_tlm_get_todays_schedules_activity( $user_id = '' ) {
    $results  = [];
    $db       = new \WeDevs\ORM\Eloquent\Database();
    $activity = new WeDevs\ERP\TLM\Models\Activity();

    $res = \WeDevs\ERP\TLM\Models\Activity::with( [
        'course' => function ( $query ) {
            $query->with( 'types' );
        }
    ...
    foreach ( $res as $key => $result ) {
        $results[ $key ]                     = $result;
        $results[ $key ]['course']['types'] = wp_list_pluck( $results[ $key ]['course']['types'], 'name' );
 ...
 * @return array
 */
function erp_tlm_get_next_seven_day_schedules_activities( $user_id = '' ) {
    global $wpdb;
    $results  = [];
    $db       = new \WeDevs\ORM\Eloquent\Database();
    $activity = new WeDevs\ERP\TLM\Models\Activity();
...
/**
 * Save email activity & send to course owner
 *
 * @param  array $email
 * @param  string $inbound_email_address
 *
 * @return array erp_tlm_save_customer_feed_data
 */
function erp_tlm_save_email_activity( $email, $inbound_email_address ) {
    ...
    $customer_feed_data = erp_tlm_save_customer_feed_data( $save_data );

    $course_id = (int) $save_data['user_id'];
    $sender_id  = $save_data['created_by'];

    $course = new \WeDevs\ERP\TLM\Course( $course_id );

    $course_owner_id = $course->get_course_owner();
    $course_owner    = get_userdata( $course_owner_id );

    // Send an email to course owner
    if ( isset( $course_owner_id ) ) {
        $to_email = $course_owner->user_email;
...
/**
 * Save email activity by course owner & send to course
 *
 * @param  array $email
 * @param  string $inbound_email_address
 *
 ...
 */
function erp_tlm_save_course_owner_email_activity( $email, $inbound_email_address ) {
    ...
    $course_id = intval( $save_data['user_id'] );
 ...
 * @return array
 */
function erp_tlm_prepare_calendar_schedule_data( $schedules ) {
    $schedules_data = [];
 ...
 * @return array
 */
function erp_tlm_get_schedule_data( $tab = '' ) {
    $args = [
        'number' => - 1,
    ...
    /**
     * If user is not a TLM Manager then he/she should always see only activities assigned to him/her.
     * For TLM Managers, in "My Schedules" tab should only show the activities assigned to him/her.
     * "All Schedules" should show all activities
     */
    if ( ! current_user_can( erp_tlm_get_manager_role() ) || 'own' === $tab ) {
...
/**
 * Get TLM email from address.
 *
 * @since 1.0
 *
 * @return string
 */
function erp_tlm_get_email_from_address() {
...
/**
 * Get TLM email from name.
 *
 * @since 1.0
 *
 * @return string
 */
function erp_tlm_get_email_from_name() {
    global $current_user;
 ...
 * @return void
 */
function erp_tlm_track_email_opened() {
    if ( isset( $_GET['aid'] ) ) {
        $activity = \WeDevs\ERP\TLM\Models\Activity::find( sanitize_text_field( wp_unslash( $_GET['aid'] ) ) );
        $extra    = json_decode( base64_decode( $activity->extra ), true );
...
/**
 * Course_Forms_Integration class instance using erp_tlm_loaded hook
 *
 * @since  1.0
 *
 * @return void
 */
function erp_tlm_course_forms() {
    // do not proceed if TLM is not active
    if ( ! wperp()->modules->is_module_active( 'tlm' ) ) {
        return;
    }

    new \WeDevs\ERP\TLM\CourseForms\CF7();
    new \WeDevs\ERP\TLM\CourseForms\Ninja_Forms();
    \WeDevs\ERP\TLM\CourseForms\Course_Forms_Integration::init();
}
 ...
 * @return array
 */
function erp_tlm_settings_pages_course_forms( $settings ) {
    if ( erp_tlm_is_current_user_manager() ) {
        $settings[] = \WeDevs\ERP\TLM\CourseForms\ERP_Settings_Course_Forms::init();
    }

    return $settings;
}

function erp_tlm_settings_pages( $settings ) {
    if ( erp_tlm_is_current_user_manager() ) {
        $settings[] = new \WeDevs\ERP\TLM\TLM_Settings();
    }
...
/**
 * Get TLM users with different params
 *
 * @since 1.0
 *
 * @param  array $args
 ...
 * @return array
 */
function erp_tlm_get_tlm_user( $args = [] ) {
    global $wp_version;
    ...
    if ( ! empty( $args['role__in'] ) ) {
        ...
        $user_query_args['role__in'] = [ 'erp_tlm_manager', 'erp_tlm_agent' ];
    }
    ...
    $tlm_user_query = new \WP_User_Query( apply_filters( 'erp_tlm_get_tlm_user_query', $user_query_args, $args ) );

    $tlm_users = $tlm_user_query->get_results();

    return $tlm_users;
}

/**
 * Get tlm user for dropdown
 *
 * @since 1.0
 *
 * @param  array $label
 ...
 * @return array
 */
function erp_tlm_get_tlm_user_dropdown( $label = [] ) {
 ...
 * @return array
 */
function erp_tlm_get_tlm_teacher_dropdown( $label = [] ) {
    $args = ['role__in' => [ 'erp_tlm_teacher' ]];
    $users = erp_tlm_get_tlm_user($args);
    $list  = [];
 ...
 * @return array
 */
function erp_tlm_get_tlm_student_dropdown( $label = [] ) {
    $args = ['role__in' => [ 'erp_tlm_teacher' ]];
    $users = erp_tlm_get_tlm_user($args);
    $list  = [];
 ...
 * @return array
 */
function erp_tlm_activity_schedule_notification_type() {
    return apply_filters( 'erp_tlm_activity_schedule_notification_type', [
        'email' => __( 'Email', 'erp' )
    ] );
}
 ...
 * @return boolean
 */
function erp_tlm_insert_save_replies( $args = [] ) {
    if ( ! $args ) {
    ...
    $save_replies = WeDevs\ERP\TLM\Models\Save_Replies::firstOrNew( [ 'id' => $args['id'] ] );
...
function erp_tlm_get_save_replies_shortcodes() {
    return apply_filters( 'erp_tlm_get_save_replies_shortcodes', [
        '{first_name}'  => [
            'title'   => __( 'First Name', 'erp' ),
            'key'     => 'first_name',
            'is_meta' => false
 ...
 * @return object
 */
function erp_tlm_get_save_replies( $args = [] ) {
    $defaults = [
        'number'  => - 1,
        'offset'  => 0,
        'orderby' => 'id',
    ...
    $args      = wp_parse_args( $args, $defaults );
    $cache_key = 'erp-tlm-save-replies-' . md5( serialize( $args ) );
    $items     = wp_cache_get( $cache_key, 'erp' );

    if ( false === $items ) {
        $results      = [];
        $save_replies = new WeDevs\ERP\TLM\Models\Save_Replies();
        ...
        // Check if args count true, then return total count customer according to above filter
        if ( $args['count'] ) {
            $items = WeDevs\ERP\TLM\Models\Save_Replies::count();
        }
 ...
 * @return array|object
 */
function erp_tlm_get_save_replies_by_id( $id ) {
    if ( empty( $id ) ) {
        return new WP_Error( 'no-record', __( 'No record found', 'erp' ) );
    }

    if ( is_array( $id ) ) {
        return WeDevs\ERP\TLM\Models\Save_Replies::whereIn( 'id', $id )->get()->toArray();
    } else {
        return WeDevs\ERP\TLM\Models\Save_Replies::find( $id )->toArray();
    }
}
 ...
 * @return boolean
 */
function erp_tlm_save_replies_delete( $id ) {
    if ( empty( $id ) ) {
        return new WP_Error( 'no-record', __( 'No record found', 'erp' ) );
    }

    if ( is_array( $id ) ) {
        return WeDevs\ERP\TLM\Models\Save_Replies::destroy( $id );
    } else {
        return WeDevs\ERP\TLM\Models\Save_Replies::find( $id )->delete();
    }
}
 ...
 * @return array
 */
function erp_tlm_render_save_replies( $template_id, $course_id ) {
    if ( empty( $template_id ) ) {
        return new WP_Error( 'no-template', __( 'No template found', 'erp' ) );
    }

    if ( empty( $course_id ) ) {
        return new WP_Error( 'no-course', __( 'No course found', 'erp' ) );
    }

    $courses       = new \WeDevs\ERP\TLM\Course( $course_id );
    $templates      = (object) erp_tlm_get_save_replies_by_id( $template_id );
    $shortcodes     = erp_tlm_get_save_replies_shortcodes();
    $courses_info  = $courses->data ;
    $data = [];

    foreach ( $shortcodes as $shortcode => $shortcode_val ) {
        if ( $shortcode_val['is_meta'] ) {
            $data[] = erp_people_get_meta( $course_id, $shortcode_val['key'], true );
        } else {
            if ( property_exists( $courses_info, $shortcode_val['key'] ) ) {
                if ($shortcode == '%country%') {
                    $data[] = erp_get_country_name($courses_info->{$shortcode_val['key']});
                } elseif ($shortcode == '%state%') {
                    $data[] = erp_get_state_name($courses_info->country, $courses_info->{$shortcode_val['key']});
                } else {
                    $data[] = $courses_info->{$shortcode_val['key']};
                }
            }
        }
    }
    ...
    $replace = apply_filters( 'erp_tlm_filter_course_data_via_shortcodes', $data, $courses );
 ...
 * @return void
 */
function erp_tlm_user_bulk_actions() {
    ?>
    <script type="text/javascript">
        jQuery(document).ready(function ($) {
            $('<option>').val('tlm_course').text('<?php esc_html_e( 'Import into TLM', 'erp' )?>').appendTo("select[name='action']");
            $('<option>').val('tlm_course').text('<?php esc_html_e( 'Import into TLM', 'erp' )?>').appendTo("select[name='action2']");
        });
    </script>
    <?php
}
 ...
 * @return void
 */
function erp_tlm_handle_user_bulk_actions() {
    $wp_list_table = _get_list_table( 'WP_Users_List_Table' );
    $action        = $wp_list_table->current_action();

    if ( ! in_array( $action, [ 'tlm_course', 'process_tlm_course' ] ) ) {
        return;
    }
    ...
    switch ( $action ) {
        case 'tlm_course':
            ...
            include( WPERP_TLM_VIEWS . '/import-user-to-tlm.php' );
        ...
        case 'process_tlm_course':
            if ( isset( $_REQUEST['_wpnonce'] ) && ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['_wpnonce'] ) ), 'erp_create_course_from_user' ) ) {
                exit;
            }
            ...
            $course_owner = isset( $_POST['course_owner'] ) ? sanitize_text_field( wp_unslash( $_POST['course_owner'] ) ) : [];

            $courses = erp_get_people_by( 'user_id', $user_ids );

            if ( ! empty( $courses ) ) {
                $course_ids = wp_list_pluck( $courses, 'user_id' );
                $user_ids    = array_diff( $user_ids, $course_ids );
            }
            ...
            foreach ( $user_ids as $user_id ) {
                ...
                $course_id = erp_insert_people( $data );

                if ( is_wp_error( $course_id ) ) {
                    continue;
                } else {
                    erp_tlm_update_course_owner( $user_id, $course_owner );
                }
 ...
 * @return void
 */
function erp_tlm_user_bulk_actions_notices() {
    global $pagenow;

    if ( $pagenow == 'users.php' && isset( $_REQUEST['created'] ) && (int) $_REQUEST['created'] ) {
        $message = wp_kses_post( sprintf( __( '%s courses created.', 'erp' ), number_format_i18n( sanitize_text_field( wp_unslash( $_REQUEST['created'] ) ) ) ) );
        echo wp_kses_post( "<div class='updated'><p>{$message}</p></div>" );
    }
}

/**
 * Create course from created user.
 *
 * @since 1.0
 * @since 1.2.8 erp_tlm_course_created action
 *
 * @param  int $user_id
 *
 * @return void
 */
function erp_tlm_create_course_from_created_user( $user_id ) {
    $user_auto_import = (int) erp_get_option( 'user_auto_import', 'erp_settings_erp-tlm_courses', 0 );

    if ( ! $user_auto_import ) {
        return;
    }

    $default_roles = erp_get_option( 'user_roles', 'erp_settings_erp-tlm_courses', [] );
    ...
    $course_owner = erp_get_option( 'course_owner', 'erp_settings_erp-tlm_courses', null );
    $course_owner = ( $course_owner ) ? $course_owner : get_current_user_id();
    $life_stage    = erp_get_option( 'life_stage', 'erp_settings_erp-tlm_courses', 'Learning' );
    ...
    $data['course_owner'] = $course_owner;
    $data['life_stage']    = $life_stage;
    $course_id            = erp_insert_people( $data );

    do_action( 'erp_tlm_course_created', $course_id, $data );
 ...
 * @return void
 */
function erp_tlm_check_new_inbound_emails() {
    ...
    try {
        ...
        $last_checked = get_option( 'erp_tlm_inbound_emails_last_checked', date( "d M Y" ) );
        ...
        update_option( 'erp_tlm_inbound_emails_last_checked', date( "d M Y" ) );

        $emails = $imap->get_emails( "Inbox", "UNSEEN SINCE \"$date\"" );

        do_action( 'erp_tlm_new_inbound_emails', $emails );
        ...
        foreach ( $emails as $email ) {
            if ( isset( $email['headers']['References'] ) && preg_match( '/<' . $email_regexp . '>/', $email['headers']['References'], $matches ) ) {
                ...
                $g_uploader           = new \WeDevs\ERP\TLM\Gmail_Sync();
                $email['attachments'] = $g_uploader->save_attachments( $email['attachments'] );
                /*** Save uploaded files end *****/
                // Save & sent the email
                switch ( $message_id_parts[3] ) {
                    case 'r1':
                        $customer_feed_data = erp_tlm_save_email_activity( $email, $imap_options['username'] );
                        break;
                    case 'r2':
                        $customer_feed_data = erp_tlm_save_course_owner_email_activity( $email, $imap_options['username'] );
                        break;
                }

                $type          = ( $message_id_parts[3] == 'r2' ) ? 'owner_to_course' : 'course_to_owner';
                $email['type'] = $type;

                do_action( 'erp_tlm_course_inbound_email', $email, $customer_feed_data );
            }
        }
...
/**
 * Get the course sources
 *
 * @return array
 */
function erp_tlm_course_sources() {
    $sources = array(
        'advert'             => __( 'Advertisement', 'erp' ),
        'chat'               => __( 'Chat', 'erp' ),
        'course_form'       => __( 'Course Form', 'erp' ),
    ...
    return apply_filters( 'erp_tlm_course_sources', $sources );
}

/**
 * Get course source dropdown
 *
 * @since 1.0.0
 *
 * @return void
 **/
function erp_tlm_course_source_dropdown( $selected = '' ) {
...
/**
 * Get course all meta fields
 *
 * @since 1.1.7
 *
 * @return array
 */
function erp_tlm_get_course_meta_fields() {
    // core meta keys
    $core_fields = [
        'life_stage',
        'course_owner',
    ...
    $social_fields = array_keys( erp_tlm_get_social_field() );

    return apply_filters( 'erp_tlm_course_meta_fields', array_merge( $core_fields, $social_fields ) );
}
 ...
 * @return void
 */
function erp_tlm_sync_people_meta_data( $meta_id, $object_id, $meta_key, $_meta_value ) {
 ...
 * @return void
 **/
function erp_tlm_make_wp_user( $customer_id, $args = [] ) {

    if ( ! erp_tlm_current_user_can_make_wp_user() ) {
    ...
    $userdata = apply_filters( 'erp_tlm_make_wpuser_args', $userdata );
    ...
    unset( $people['id'], $people['user_id'], $people['website'], $people['email'], $people['created'], $people['types'], $people['last_name'], $people['first_name'], $people['life_stage'], $people['course_owner'] );
...
/**
 * WP user on delete update course user id
 *
 * @since 1.1.7
 *
 * @return void
 **/
function erp_tlm_course_on_delete( $user_id, $hard = 0 ) {
...
/**
 * Get default course owner
 *
 * @since 1.1.17
 *
 * @return int
 */
function erp_tlm_get_default_course_owner() {
    $course_owner = erp_get_option( 'course_owner', 'erp_settings_erp-tlm_courses', 0 );
 ...
 * @return boolean
 */
function erp_tlm_wc_prevent_admin_access( $prevent_access ) {
 ...
 * @param string $selected
 */
function erp_tlm_dropdown_roles( $selected = '' ) {
    $r = '';
...
/**
 * Redirect tlm role based user to their page
 *
 * @since 1.2.5
 *
 * @param $redirect_to
 ...
 * @return string
 */
function erp_tlm_login_redirect( $redirect_to, $roles ) {
    $tlm_manager = erp_tlm_get_manager_role();
    $tlm_agent   = erp_tlm_get_agent_role();
    $tlm_teacher = erp_tlm_get_teacher_role();

    if ( in_array( $tlm_manager, $roles ) || in_array( $tlm_agent, $roles ) || in_array( $tlm_teacher, $roles ) ) {
        $redirect_to = get_admin_url( null, 'admin.php?page=erp-tlm' );
    }
 ...
 */
function erp_tlm_get_life_stage( $course_id ) {
    $course = new \WeDevs\ERP\TLM\Course( $course_id );
 ...
 */
function erp_tlm_update_life_stage( $course_id, $stage ) {
    $course = new \WeDevs\ERP\TLM\Course( $course_id );
    ...
    return $course->update_life_stage( $stage );
}
 ...
 */
function erp_tlm_get_course_owner( $course_id ) {
    $course = new \WeDevs\ERP\TLM\Course( $course_id );
    ...
    return $course->get_course_owner();
}
 ...
 */
function erp_tlm_update_course_owner( $course_id, $owner_id, $field_type = 'user_id' ) {
    $people = erp_get_people_by( $field_type, $course_id );

    if ( empty( $people ) ) {
        return new \WP_Error( 'no-erp-people', __( 'People not exists', 'erp' ) );
    }

    $course = new \WeDevs\ERP\TLM\Course( $people->id );

    $course->update_course_owner( $owner_id );
}
 ...
 * @return array
 */
function erp_tlm_get_course_groups_list() {
    $groups         = \WeDevs\ERP\TLM\Models\CourseGroup::select( 'id', 'name' )->get();
    $course_groups = apply_filters( 'erp_tlm_get_course_group_list', $groups );
 ...
 * @return string|WP_Error
 */
function erp_tlm_get_course_hash( $course_id ) {
    $course = new \WeDevs\ERP\TLM\Course( $course_id );
 ...
 * @return WP_Error|void
 */
function erp_tlm_update_course_hash( $course_id, $hash ) {
    $course = new \WeDevs\ERP\TLM\Course( $course_id );
    ...
    $course->update_course_hash( $hash );
}
 ...
 *
 */
function erp_tlm_get_course_tags( $list = true ) {
    $options = [];
    $terms   = get_terms( array(
        'taxonomy'   => 'erp_tlm_tag',
        'hide_empty' => false,
    ) );
 ...
 * @return void
 */
function erp_tlm_add_tag_taxonomy(){
    new \WeDevs\ERP\TLM\Course_Taxonomy( 'erp_tlm_tag', 'erp_tlm_tag', array(
        'singular' => __( 'Tag',  'erp' ),
        'plural'   => __( 'Tags', 'erp' ),
        'show_ui'   => false,
    ) );
 ...
 * @return bool
 */
function erp_tlm_sync_is_active() {
    if ( wperp()->google_auth->is_active() ) {
        return true;
    }
...
/**
 * Send birthday greetings to course
 *
 * @return void
 */
function erp_tlm_send_birthday_greetings() {
        $email =  new WeDevs\ERP\TLM\Emails\Birthday_Greetings();
        $email->trigger();
}

/**
 * Check if course and company has relation
 *
 * @return void
 */
function erp_tlm_check_company_course_relations($id, $id_type) {
    global $wpdb;
    ...
    if ( isset( $id ) && isset( $id_type ) ) {
        if ( !empty( $id_type ) ) {
            ...
            $rel_count = $wpdb->get_var( "SELECT count(*) FROM {$wpdb->prefix}erp_tlm_customer_companies WHERE {$id_type}_id = {$id}" );
            return $rel_count ;
        }
    }
}
