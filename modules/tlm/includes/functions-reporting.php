 ...
 * @return array
 */
function erp_tlm_get_reports() {

    $reports = [
        'activity-report' => [
            'title'       => esc_html__( 'Activity Report', 'erp' ),
            'description' => esc_html__( 'Activity report for tlm', 'erp' )
        ],
        'customer-report' => [
            'title'       => esc_html__( 'Customer Report', 'erp' ),
            'description' => esc_html__( 'Customer report for tlm', 'erp' )
        ],
        'growth-report'   => [
            'title'       => esc_html__( 'Growth Report', 'erp' ),
            'description' => esc_html__( 'Growth report for tlm', 'erp' )
        ],
    ];

    return apply_filters( 'erp_tlm_reports', $reports );
}
 ...
 *
 */
function erp_tlm_activity_report_filter_form( $start = true, $end = true ) {
    if ( ! isset( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_key( $_REQUEST['_wpnonce'] ), 'erp_tlm_nonce_report' ) ) {
        return;
    }
    ...
    echo '<form class="erp-tlm-report-filter-form" action="" method="post">';
    ...
    wp_nonce_field( 'erp_tlm_nonce_report' );

    submit_button( esc_html__( 'Filter', 'erp' ), 'secondary', 'erp_tlm_report_filter', false );
 ...
 *
 */
function erp_tlm_customer_report_filter_form( $start = true, $end = true, $type = false ) {
    if ( ! isset( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_key( $_REQUEST['_wpnonce'] ), 'erp_tlm_nonce_report' ) ) {
        return;
    }
    ...
    erp_html_form_input( array(
            ...
            'type'        => 'select',
            'class'       => 'filter-types',
            'id'          => 'erp-tlm-select-types',
            'options'     => [
                'life_stage'    => esc_html__( 'All Types', 'erp' ),
                'course_owner' => esc_html__( 'Owner Wise', 'erp' ),
 ...
 *
 */
function erp_tlm_growth_report_filter_form( $start = true, $end = true, $type = false ) {
    if ( ! isset( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_key( $_REQUEST['_wpnonce'] ), 'erp_tlm_nonce_report' ) ) {
        return;
    }
 ...
 *
 */
function erp_tlm_activity_reporting_query( $start_date, $end_date ) {
    $activities = \WeDevs\ERP\TLM\Models\Activity::select( 'type', \WeDevs\ORM\Eloquent\Facades\DB::raw( 'count(*) as total' ) );
...
function tlm_customer_report_query_helper( $filter_type, $id ) {
 ...
 *
 */
function erp_tlm_customer_reporting_query( $start_date, $end_date, $filter_type ) {
    ...
    switch ( $filter_type ) {
        ...
        case 'group':
            ...
            $results = $wpdb->get_results( "SELECT cg.name, p.life_stage FROM `{$wpdb->prefix}erp_tlm_course_subscriber` cs LEFT JOIN `{$wpdb->prefix}erp_peoples` p ON cs.user_id = p.id LEFT JOIN `{$wpdb->prefix}erp_tlm_course_group` cg ON cs.group_id = cg.id {$where}", OBJECT );
    ...
    switch ( $filter_type ) {
        ...
        case 'course_owner':
            foreach ( $results as $key => $people ) {
                $id = $people->course_owner;

                if ( $id ) {
                    $course                        = get_user_by( 'id', $people->course_owner );
                    $results[ $key ]->course_owner = $course->display_name;

                    $results[ $key ]->owner_data = tlm_customer_report_query_helper( $filter_type, $id );
                }
            }
        ...
        case 'country':
            foreach ( $results as $key => $result ) {
                ...
                if ( $id ) {
                    $results[ $key ]->country_data = tlm_customer_report_query_helper( $filter_type, $id );
                }
            }
 ...
 *
 */
function erp_tlm_growth_reporting_query( $start_date, $end_date, $type ) {
    $temp_array = [];
    $reports    = [];
    ...
    wp_localize_script( 'erp-tlm-report', 'growthReport', [
        'type'    => $type,
        'reports' => $reports
    ] );
