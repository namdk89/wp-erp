...
<?php
$feeds_tab = erp_tlm_get_customer_feeds_nav();
$tlm_users = erp_tlm_get_tlm_user();
?>
<div class="wrap erp erp-tlm-activities erp-single-customer" id="wp-erp">

    <h2><?php esc_attr_e( 'Activities', 'erp' ); ?></h2>
    <hr>
    <div class="erp-customer-feeds" id="erp-customer-feeds" v-cloak>

        <div class="activity-filter" id="erp-tlm-activities-filter">
            ...
            <?php if ( ! current_user_can( 'erp_tlm_agent' ) ) : ?>
            <div class="filters">
                <select style="width:260px;" v-selecttwo="filterFeeds.created_by" class="select2" v-model="filterFeeds.created_by" id="activity-created-by" data-placeholder="<?php esc_attr_e( 'Created by..', 'erp' ) ?>">
                    ...
                    <?php foreach ( $tlm_users as $tlm_user ) : ?>
                        <option value="<?php echo esc_attr( $tlm_user->ID ) ?>"><?php echo esc_attr( $tlm_user->display_name ); ?></option>
                    <?php endforeach ?>
                </select>
            </div>
            <?php endif ?>
            ...
            <div class="filters">
                <select style="width:260px;" v-selecttwo="filterFeeds.customer_id" data-types="course,company"  class="erp-tlm-course-list-dropdown" v-model="filterFeeds.customer_id" id="activity-created-for"  data-placeholder="<?php esc_attr_e( 'Created for course or company ..', 'erp' ) ?>">
                    <option value=""></option>
                </select>
            </div>
        ...
        <div class="activity-content">
            ...
            <ul class="timeline" v-if = "feeds.length">
                ...
                <template v-for="( month, feed_obj ) in feeds | formatFeeds">
                    ...
                    <li v-for="feed in feed_obj">
                        ...
                        <?php do_action('erp_add_custom_content_after_feed', '{{ feed.course.id }}' );?>
                    </li>
            ...
            <div class="no-activity-found" v-if="!feeds.length">
                <?php esc_attr_e( 'No Activity found for this Course', 'erp' ); ?>
            </div>
