<div class="erp-tlm-new-schedule-wrapper">
        ...
        <div class="feed-schedule-wrapper">
            ...
            <div class="schedule-title-assign-user">
                ...
                <p class="erp-left schedule-assign-user">
                    <select name="user_id" required class="erp-tlm-course-list-dropdown" id="assign-course" style="width: 100%" data-types="course,company" data-placeholder="<?php esc_attr_e( 'Assign to a course..', 'erp' ) ?>">
                        <option value=""></option>
                    </select>
                </p>
            ...
            <p>
                <select name="invite_course[]" id="erp-tlm-activity-invite-course" class="select2" multiple="multiple" style="width: 100%" data-placeholder="<?php esc_attr_e( 'Agents or managers..', 'erp' ) ?>">
                    <?php echo wp_kses( erp_tlm_get_tlm_user_html_dropdown(), array(
                        'option' => array(
                            'value' => array(),
                            'selected' => array()
                        ),
    ...
    <# } else { #>
        <div class="feed-log-activity">
            <p>
                <select required name="user_id" class="erp-tlm-course-list-dropdown" id="assign-course"  data-types="course,company" style="width: 100%" data-placeholder="<?php esc_attr_e( 'Assign to a course..', 'erp' ) ?>">
                    <option value=""></option>
                </select>
            </p>

            <p>
                <select name="log_type" required id="erp-tlm-feed-log-type" class="erp-left">
            ...
            <p class="log-selected-course erp-hide">
                <select name="invite_course[]" id="erp-tlm-activity-invite-course" class="select2" multiple="multiple" style="width: 100%" data-placeholder="<?php esc_attr_e( 'Agents or managers..', 'erp' ) ?>">
                    <?php echo wp_kses( erp_tlm_get_tlm_user_html_dropdown(), array(
                        'option' => array(
                            'value' => array(),
    ...
    <div class="submit-action">
        <input type="hidden" name="action" value="erp_tlm_add_schedules_action">
        <input type="hidden" name="created_by" value="<?php echo esc_attr( get_current_user_id() ); ?>" >
        <?php wp_nonce_field( 'wp-erp-tlm-customer-feed' ); ?>
    </div>
</div>
