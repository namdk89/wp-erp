<div id="erp-tlm-single-schedule-details-wrap">
    <p class="title">
        <# if( data.schedule.created_by.ID == wpErpTml.current_user_id ) { #>
        ...
        for
        <# if( _.contains( data.schedule.course.types , 'company') ) { #>
            <strong>{{ data.schedule.course.company }}</strong>
        <# } else { #>
            <strong>{{ data.schedule.course.last_name }} {{ data.schedule.course.first_name }}</strong>
        <# } #>
        ...
        <# } else { #>
            <# if( data.schedule.extra.invited_user.length > 1 ) { #>
                ...
                <# var users = data.schedule.extra.invited_user.map( function( elm ) {
                    if ( elm.id == wpErpTml.current_user_id ) {
            ...
            <# } else if ( data.schedule.extra.invited_user.length == 1 ) { #>
                <# if ( data.schedule.extra.invited_user[0].id == wpErpTml.current_user_id ) {
                        var users = 'You';
                    } else {
