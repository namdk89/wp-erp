...
$schedules_data = erp_tlm_get_schedule_data( $tab );
?>
<div class="wrap erp erp-tlm-schedules" id="wp-erp">

    <h1><?php esc_attr_e( 'Schedules', 'erp' ); ?></h1>

    <?php if ( current_user_can( erp_tlm_get_manager_role() ) ): ?>
        <h2 class="nav-tab-wrapper erp-nav-tab-wrapper">
            <a class="nav-tab <?php echo $tab == 'own' ? 'nav-tab-active': ''; ?>" href="<?php echo esc_url_raw( add_query_arg( [ 'page'=>'erp-tlm', 'section' => 'schedules', 'tab' => 'own' ], admin_url( 'admin.php' ) ) ); ?>"><?php esc_attr_e( 'My Schedules', 'erp' ); ?></a>
            <a class="nav-tab <?php echo $tab == 'all' ? 'nav-tab-active': ''; ?>" href="<?php echo esc_url_raw( add_query_arg( [ 'page'=>'erp-tlm', 'section' => 'schedules', 'tab' => 'all' ], admin_url( 'admin.php' ) ) ); ?>"><?php esc_attr_e( 'All Schedules', 'erp' ); ?></a>
        </h2>
    <?php endif; ?>


    <div class="erp-tlm-schedule-wrapper">
        <div id="erp-tlm-schedule-calendar"></div>
    </div>
</div>
...
<script>
    ;jQuery(document).ready(function($) {
        $('#erp-tlm-schedule-calendar').fullCalendar({
            header: {
                ...
                $.erpPopup({
                    ...
                    onReady: function() {
                        ...
                        wp.ajax.send( 'erp-tlm-get-single-schedule-details', {
                            data: {
                                id: scheduleId,
                                _wpnonce: '<?php echo esc_html( wp_create_nonce( 'wp-erp-tlm-nonce' ) ); ?>'
                            },
            ...
            dayClick: function(date, jsEvent, view) {
                ...
                $.erpPopup({
                    ...
                    id: 'erp-tlm-customer-schedules',
                    content: wperp.template('erp-tlm-customer-schedules')( { current_date: date.format() } ).trim(),
                    extraClass: 'larger',
                    onReady: function () {
                        var modal = this;
                    ...
                    onSubmit: function(modal) {
                        ...
                        wp.ajax.send( {
                            ...
                            success: function( res ) {
                                ...
                                $('#erp-tlm-schedule-calendar').fullCalendar( 'renderEvent', myEvent );
